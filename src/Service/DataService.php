...
class DataService
{
    ...
    public function returnCasaData(Request $request)
    {
        ...
        $query = $em->createQuery(
            'SELECT u,p FROM App\Entity\Casa u JOIN u.images  p ORDER BY u.valoracion'
        );
    ...
    public function returnExcursionData(Request $request)
    {
        ...
        $container = $this->container;
        $query = $em->createQuery(
            'SELECT u,p FROM App\Entity\Excursion u JOIN u.images  p ORDER BY u.valoracion'
        );
        $paginator = $container->get('knp_paginator');
        $results = $paginator->paginate(
            $query,
    ...
    public function returnCarroData(Request $request)
    {
        ...
        $container = $this->container;
        $query = $em->createQuery(
            'SELECT u,p FROM App\Entity\Carro u JOIN u.images  p ORDER BY u.valoracion'
        );
        $paginator = $container->get('knp_paginator');
        $results = $paginator->paginate(
            $query,
    ...
    public function returnPaqueteData(Request $request)
    {
        ...
        $container = $this->container;
        $query = $em->createQuery(
            'SELECT u ,p FROM App\Entity\Paquete u JOIN u.images  p ORDER BY u.valoracion'
        );
        $paginator = $container->get('knp_paginator');
        $results = $paginator->paginate(
            $query,
